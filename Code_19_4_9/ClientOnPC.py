...
##数据接收线程函数
recv_data = 0 ##暂时替代收到的数据类型，改成视频需要改
...
def motionControl(enable):
	...
	##定时发生
        ...
        if (current > 0.1):
            ##PWM数据合成，发送
            msg = ("FromPC+"+str(int(PWM_A_0)) +'+'+ str(int(PWM_A_1)) +'+'+ str(int(PWM_B_0)) +'+'+ str(int(PWM_B_1)) + '+'+str(int(PWM_C_0)) +'+'+ str(int(PWM_C_1))+ '+'+ \
                  str(int(PWM_D_0)) +'+'+ str(int(PWM_D_1)) +'+'+ str(int(PWM_E_0)) +'+'+ str(int(PWM_E_1)) +'+'+ str(int(PWM_F_0)) +'+'+ str(int(PWM_F_1)) +'+'+ "B").strip()
...
#for diaplay remain to finish ##在这里面进行数据解算和视频显示
